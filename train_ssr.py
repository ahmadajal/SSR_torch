...
import smooth_loss
from smooth_loss import BetaLoss
...
# load_from_last_check_point = False
...
def main():
    ...
    smooth_loss.config['device'] = device

    # Configure the dataset, model and the optimizer based on the global
    # `config` dictionary.
    training_loader, test_loader = get_dataset(data_root="../Robust_Explanations/notebooks/data")
    model = ResNet18()
    model.to(device)
    ...
    for epoch in range(config['num_epochs']):
        ...
        for batch_x, batch_y in training_loader:
            ...
            optimizer.zero_grad()
            # we should do the following, because in BetaLoss we need to take
            # the grad of logits wrt input.
            batch_x.requires_grad_(True)
            prediction = model(batch_x)
            ####
            criterion = BetaLoss(batch_x, prediction, alpha=config['alpha'], scale=config['scale'])
            ####
...
def get_dataset(test_batch_size=32, shuffle_train=True, num_workers=2, data_root='./data'):
    """
